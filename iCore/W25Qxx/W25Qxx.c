...
void BSP_W25Qx_Read_ID(uint16_t *ID)
{
	...
	uint8_t temp[2];//temp[0]存放Manufacture ID, temp[1]存放Device ID
	HAL_GPIO_WritePin(GPIOC,GPIO_PIN_4,GPIO_PIN_RESET);
	/* Send the read ID command */
	HAL_SPI_Transmit(&hspi2, cmd, 4, W25Qx_TIMEOUT_VALUE);	
	/* Reception of the data */
	HAL_SPI_Receive(&hspi2,temp, 2, W25Qx_TIMEOUT_VALUE);
	*ID = (temp[0]<<8)|temp[1];
	HAL_GPIO_WritePin(GPIOC,GPIO_PIN_4,GPIO_PIN_SET);
...
uint16_t W25Qx_Read_ID(void)
{
	...
	{
		if(HAL_SPI_Receive(&hspi2,temp, 2, W25Qx_TIMEOUT_VALUE)==HAL_OK)
		{
