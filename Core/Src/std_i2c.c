...
i2c_status_def i2c_bus_read_mulit(i2c_bus_class* pBus, i2c_addr_wide_def wide, uint16_t dev_addr, uint16_t reg_addr, uint16_t length, uint8_t *pData)
{
	if(pBus == NULL)
	{ return i2c_error; }
...
i2c_status_def i2c_dev_read_mulit(i2c_dev_class* pDev, uint16_t reg_addr, uint16_t length, uint8_t *pData)
{
	return i2c_bus_read_mulit((pDev->bus), pDev->addr_wide, pDev->addr, reg_addr, length, pData);
}
